...
conf.maxdupes = conf.maxdupes or 6
config.setoption("labelcolor", {
  ...
  onshow = function (defaultcolour)
    ...
    fg(conf.labelcolor or "a_cyan") echoLink(" "..(conf.labelcolor or '?'), "printCmdLine'mmconfig labelcolor '", "Click to change the color", true)
    ...
    fg(defaultcolour)
    echo("); font size is ")
    fg("a_cyan") echoLink(tostring(conf.labelsfont), "printCmdLine'mmconfig labelsfont '", "Click to set the font size for peopletracker labels", true)
    echo(".\n")
  end,
  onset = function ()
...
sys.update_people_labels = function ()
  local s,m = pcall(function()
    ...
    for room, persons in pairs(r) do
      ...
      local areanum = getRoomArea(mmp.currentroom)
      ...
      if ids and #ids >1 and #ids <= conf.maxdupes then
        ...
        local currentzlevel = select(3, getRoomCoordinates(mmp.currentroom))
        -- debugf("currentzlevel: %s", currentzlevel)
        for _, exactroomid in pairs(ids) do
          -- debugf("running with exactroomid %s", exactroomid)
          coords[exactroomid] = {getRoomCoordinates(exactroomid)}
          coords[exactroomid][4] = getRoomArea(exactroomid)
          -- debugf("coords for exactroomid: %s", pl.pretty.write(exactroomid))
          ...
          if not rightmost[coords[exactroomid][3]] or
            ...
            -- debugf("rightmost so far: %s", pl.pretty.write(rightmost))
            rightmost[coords[exactroomid][3]] =
              ...
              -- debugf("rightmost after: %s", pl.pretty.write(rightmost))
          end
        end
  ...
  if not s then
    echoLink("(e!)", string.format("echo([[The problem was: %q]])", m), 'Oy - there was a problem with the peopletracker. Click on this link and report a bug with what it says.')
  end
end
