...
signals.systemstart:connect(function () addbalanceful("m&m check do", sk.check_do) end)

function sk.check_dofree()
  if not bals.balance or not bals.equilibrium or not bals.rightarm or not bals.leftarm then return end
...
signals.systemstart:connect(function () addbalanceless("m&m check dofree", sk.check_dofree) end)
  ...
  if what then
    for i,v in ipairs(me.dofreequeue) do
      if v == what then
        ...
        if echoback then echof("Removed \"%s\" from the do queue.", v) return end
      end
    end
    ...
    if echoback then echof("Removed \"%s\" from the do queue.", what) end
  end
  raiseEvent("m&m dofree changed")
end
